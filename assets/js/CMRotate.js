...
var CMRotate = CMRotate || ( function () {
    ...
    function init(div, tw, th, ty, gap, radius, bg, fn) {
        ...
        _itemW = tw * scale; // Apply scale factor to width
        _itemH = th * scale; // Apply scale factor to height
        _ty = ty * scale; // Apply scale factor to Y position from bottom
        _itemHW = _itemW >> 1;
        _itemHH = _itemH >> 1;
        _gap = gap * scale*2; // Optionally apply scale to gap
        _radius = radius * scale; // Apply scale factor to radius
        _posTotal = Math.ceil(360 / _gap);
    
        // Positioning and reverse calculation remain unchanged
        ...
        // Mouse event, resize event, and animation frame call remain unchanged
        addMouseEvent();
        addScrollEvent();
    ...
    function addScrollEvent() {
        ...
        if (rotateDiv) {  // Check if the rotate-div element is present
        
            window.addEventListener('wheel', function(event) {
        
                var delta = event.deltaY; // Gets the vertical scroll amount
                // Adjust rotation based on the scroll direction
                _moveX += delta * 0.07; // Modify this multiplier to adjust sensitivity
            }, {passive: false});
        }
    }
    ...
    function animate() {
        ...
        _moveX *= 0.9; // Dampening the rotation speed over time
